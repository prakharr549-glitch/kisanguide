...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.log('Fetching from OpenWeather Forecast API:', finalUrl.replace(API_KEY || '', 'REDACTED'));

    const response = await fetch(finalUrl);
    
    if (!response.ok) {
      const errorText = await response.text().catch(() => '');
      console.error('OpenWeather Forecast API Error Status:', response.status);
      console.error('OpenWeather Forecast API Error Body:', errorText);
