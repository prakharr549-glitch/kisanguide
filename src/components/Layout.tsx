...
import { Home, Cloud, ShoppingBag, BookOpen, Calculator, Menu, X, Lightbulb, Newspaper, Moon, Sun } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col transition-colors duration-300">
      {/* Header */}
      <header className="bg-green-600 dark:bg-green-800 text-white shadow-lg sticky top-0 z-50">
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex items-center space-x-4">
                ...
                <Link
                  ...
                  className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                    location.pathname === item.path
                      ? 'bg-green-700 dark:bg-green-900 text-white'
                      : 'text-green-100 hover:bg-green-500 dark:hover:bg-green-700 hover:text-white'
                  }`}
                >
                  {item.name}
                </Link>
              ...
              <button
                ...
                className="p-2 rounded-full hover:bg-green-500 dark:hover:bg-green-700 transition-colors ml-2"
            ...
            <div className="flex items-center md:hidden space-x-2">
              <button
                onClick={toggleDarkMode}
                className="p-2 rounded-full hover:bg-green-500 dark:hover:bg-green-700 transition-colors"
                aria-label="Toggle Dark Mode"
              >
                {isDarkMode ? <Sun className="h-5 w-5" /> : <Moon className="h-5 w-5" />}
              </button>
              <button onClick={toggleMenu} className="p-2 rounded-md hover:bg-green-500 dark:hover:bg-green-700 focus:outline-none">
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <div className="md:hidden bg-green-600 dark:bg-green-800 border-t border-green-500 dark:border-green-700">
